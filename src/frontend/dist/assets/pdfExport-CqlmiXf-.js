const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/jspdf.es.min-C094GTmB.js","assets/index-BUSSrS1X.js","assets/index-COXDIJeC.css"])))=>i.map(i=>d[i]);
import { _ as __vitePreload, A as AnalysisMode } from "./index-BUSSrS1X.js";
const MODE_LABELS = {
...
const BG = [14, 15, 22];
const CARD = [20, 22, 32];
const SECTION_HDR = [100, 200, 220];
const TXT_PRIMARY = [230, 232, 245];
const TXT_MUTED = [130, 132, 155];
const ACCENT_AMBER = [200, 170, 60];
const LINE_COLOR = [40, 44, 60];
async function getJsPDF() {
  const { default: jsPDF } = await __vitePreload(async () => {
    const { default: jsPDF2 } = await import("./jspdf.es.min-C094GTmB.js").then((n) => n.j);
    return { default: jsPDF2 };
  }, true ? __vite__mapDeps([0,1,2]) : void 0);
  return jsPDF;
}
async function exportScanToPDF(scan) {
  var _a;
  ...
  const ts = new Date(Number(scan.timestamp / BigInt(1e6)));
  ...
  const textBlock = (text, maxW = CONTENT_W) => {
  ...
  const name = (((_a = scan.analysisResult.objects[0]) == null ? void 0 : _a.name) ?? "scan").toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
  doc.save(`visual-lens-${name}-${Date.now()}.pdf`);
}
async function exportScansToPDF(scans, title, onProgress) {
  var _a;
  ...
  doc.setTextColor(...TXT_MUTED);
  doc.text(
    `${scans.length} scan${scans.length !== 1 ? "s" : ""}  ·  Exported ${(/* @__PURE__ */ new Date()).toLocaleString()}`,
    MARGIN,
    52
  );
  let tocY = 80;
  ...
  scans.forEach((scan, i) => {
    var _a2;
    if (tocY > H - 20) return;
    const label = ((_a2 = scan.analysisResult.objects[0]) == null ? void 0 : _a2.name) ?? "Scan";
  ...
  for (let idx = 0; idx < scans.length; idx++) {
    ...
    onProgress == null ? void 0 : onProgress(idx + 1, scans.length);
    await new Promise((r) => setTimeout(r, 15));
  }
  const filename = title.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "").slice(0, 40);
  doc.save(`visual-lens-${filename || "export"}-${Date.now()}.pdf`);
}
export {
  exportScansToPDF as a,
  exportScanToPDF as e
};
